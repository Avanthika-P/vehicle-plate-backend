"""
Vehicle Number Plate Detection Backend (Tesseract / free-tier friendly)
--------------------------------------------------------------------------
...
approach lightweight enough to run on low-memory free hosting (e.g.
Railway's free tier) without needing a deep-learning OCR engine.
...
def find_region_candidates(img: np.ndarray):
    ...
    # Sobel-gradient + morphological closing: highlights dense text
    # regions specifically (better than plain edges at telling plate
    # text apart from painted body panels).
    ...
    sobel_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (22, 5))
    sobel_closed = cv2.morphologyEx(sobel_thresh, cv2.MORPH_CLOSE, sobel_kernel)
    contours3, _ = cv2.findContours(sobel_closed, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    all_boxes.extend(cv2.boundingRect(c) for c in contours3)
...
def detect_plate(img: np.ndarray):
    ...
    best_loose = None
    if is_loose_plate_like(whole_image_text):
        best_loose = {
            "text": whole_image_text,
            "box": {"x": 0, "y": 0, "width": w, "height": h},
            "match": "loose",
    ...
    for x, y, rw, rh in find_region_candidates(img):
        ...
        if best_loose is None and is_loose_plate_like(crop_text):
            best_loose = {
                "text": crop_text,
                "box": {"x": int(x), "y": int(y), "width": int(rw), "height": int(rh)},
                "match": "loose",
